...
class FlutterBluePlus {
  ...
  static Future<void> turnOn({int timeout = 60}) async {
    ...
    if (changed) {
      // wait for response
      BmTurnOnResponse response = await FutureTimeout(futureResponse).fbpTimeout(timeout, "turnOn");
      ...
      await FutureTimeout(adapterState.where((s) => s == BluetoothAdapterState.on).first).fbpTimeout(timeout, "turnOn");
    }
  }
        ...
        if (response == null) {
          ...
          for (BmScanAdvertisement bm in response.advertisements) {
            ...
            if (oneByOne) {
              ...
              AddOrUpdate(output).addOrUpdate(sr);
            }
          }
  ...
  static Future<void> turnOff({int timeout = 10}) async {
    ...
    // wait for response
    await FutureTimeout(futureResponse).fbpTimeout(timeout, "turnOff");
  }
